...
namespace VideoStreamingDownloader.ETB.Downloads
{
    ...
    {
        private readonly string OriginDescription = "ETB";
            ...
            base(CalculateMaxStatusValue(downloadOptions), downloadOptions.Id)
        {
            ...
            _ETBService = new ETBService(_downloadOptions.FileInfo.Id);
        }
        ...
        {
            Program.LoggerService.LogInformation($"[{OriginDescription}][{_downloadOptions.FileInfo.Id}] {message}");
        }
        ...
        private async Task<string> GetDecryptionKey(Track.Video video)
        {
            ...
            string key = await _ETBService.GetDecryptionKey(video);
        ...
        private async Task<(Track.Video, Tracks.Audio, Tracks.Subtitle)> GetMediaOptions()
        {
            ...
            var options = await _ETBService.GetMediaOptions();
